...
from romulo_ds_tools.datasets import delete_dataset_reports, registry_snapshot, scaffold_dataset
...
def delete_dataset(
    ...
    yes: Annotated[
        bool,
        typer.Option("--yes", "-y", help="Actually delete the listed report artifacts."),
    ] = False,
) -> None:
    results = delete_dataset_reports(dataset, dry_run=not yes)
    if not results:
        typer.echo(f"No report paths configured for dataset {dataset!r}")
        return

    if not yes:
        typer.echo("Dry run. No files were deleted. Re-run with --yes to delete:")
    for result in results:
        status = "deleted" if result.deleted else "exists" if result.existed else "missing"
